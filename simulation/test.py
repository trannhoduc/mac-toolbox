...
class Station:
    ...
    def generate_report(self):
        ...
        throughput = self.successful_transmissions / self.steady_state_time
        print(f"Station: {self.name}")
        print(f"  Mean transmit time: {self.T:.4f}")
        print(f"  Mean retries: {self.mean_retries:.4f}")
        print(f"  Utilization: {self.U * 100:.2f}%")
        print(f"  Throughput: {throughput:.4f}")
        print(f"  Initial messages: {self.num_initial_transmits}")
        print(f"  Retries: {self.num_retries}\n")
    ...
    def arrive(self):
        ...
        while True:
            ...
            self.env.process(self.transmit(f'Frame {i}'))
            i += 1

def generate_report_single_replication(mean_transmit_times, mean_num_retries, channel_utilizations, stations):
    total_st, total_nt, total_retries, total_initial_transmits, total_busy_time, total_successful_transmissions = 0, 0, 0, 0, 0, 0
    ...
    for station in stations:
        ...
        total_successful_transmissions += station.successful_transmissions
    ...
    throughput = total_successful_transmissions / STEADY_STATE_TIME
    ...
    print(f"  Mean transmit time: {mean_t:.4f}")
    print(f"  Mean retries: {mean_r:.4f}")
    print(f"  Channel utilization: {mean_U * 100:.2f}%")
    print(f"  Throughput: {throughput:.4f}\n")
...
def generate_report_all_replications(mean_transmit_times, mean_num_retries, channel_utilizations):
    ...
    print("-----------------------")
    print("Overall Report Across All Replications:")
    print(f"  Mean transmit time: {mean_t:.4f}")
    print(f"  Mean retries: {mean_r:.4f}")
    print(f"  Channel utilization: {mean_U * 100:.2f}%")
